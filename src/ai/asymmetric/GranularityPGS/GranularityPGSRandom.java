 ...
 */
public class GranularityPGSRandom extends AIWithComputationBudget implements InterruptibleAI {
	...
	public GranularityPGSRandom(int time, int max_playouts, int la, int a_I, int a_R, EvaluationFunction e,
	...
	@Override
	public AI clone() {
		return new GranularityPGSRandom(TIME_BUDGET, ITERATIONS_BUDGET, LOOKAHEAD, I, R, evaluation, utt, pf);
	}
	...
	public PlayerAction getFinalAction(UnitScriptData currentScriptData) throws Exception {
		 ...
		 * currentScriptData.getAIUnit(u); UnitAction unt =
		 * actions.get(ai.toString()).getAction(u);
		 * 
		 * if (unt.equals(null)) { pAction.addUnitAction(u, unt); } else {
