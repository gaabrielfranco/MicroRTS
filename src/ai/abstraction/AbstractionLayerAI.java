...
public abstract class AbstractionLayerAI extends AIWithComputationBudget {
	...
	// set this to true, if you believe there is a bug, and want to verify that
	// actions
	...
	// 1) You can forget about issuing actions to all units, just issue the ones you
	// want, the rest are set to NONE automatically
	...
	// In case the GameState is cloned, and the Unit pointers in the "actions" map
	// change, this variable
	// saves a pointer to the previous GameState, if it's different than the current
	// one, then we need to find a mapping
	...
	public int findBuildingPosition(List<Integer> reserved, int desiredX, int desiredY, Player p,
		 ...
		 * for(int i = 0;i<free[0].length;i++) { for(int j = 0;j<free.length;j++) {
		 * System.out.print(free[j][i] + "\t"); } System.out.println(""); }
		 */
